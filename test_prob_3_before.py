...
def heat_maps_adjacency_matrix(GT, test, test_p, title):
    fig, axes = plt.subplots(1, 3, figsize=(20, 10))  # Create 1 row, 2 columns


    axes[0].imshow(GT)
    sns.heatmap(GT, linewidths=1, ax=axes[0])
    axes[0].set_title('GT - '+title)


    axes[1].imshow(test)
    sns.heatmap(test, linewidths=1, ax=axes[1])
    axes[1].set_title('LLM - '+title)

    axes[2].imshow(test_p)
    sns.heatmap(test_p, linewidths=1, ax=axes[2])
    axes[2].set_title('LLM p - ' + title)

    plt.tight_layout()  # Adjust layout for better spacing
    plt.savefig('GT_vs_LLM_'+title+'.png')  # Save the figure
    plt.show()
...
data = {
    "steps": {
        ...
        "2": "Serve the pancakes with chopped strawberries",
        "3": "Melt a small knob of butter in a non-stick frying pan over low-medium heat",
        "4": "splash maple syrup on plate",
        "5": "Add 1 banana to a blender",
        "6": "Cook for 1 min or until the tops start to bubble",
        "7": "blitz-blitz the blender for 20 seconds",
        "8": "Flip the pancakes with a fork or a fish slice spatula",
        "9": "Add-1 egg to a blender",
        "10": "cook for 20-30 seconds more",
        "11": "Pour three little puddles straight from the blender into the frying pan",
        "12": "Add-1 heaped tbsp flour to a blender",
        "13": "Chop 1 strawberry",
...
output_folder_test =  r"C:\Users\Sveta\PycharmProjects\LLM_check\Cook_test_March"
...
for file_name_config in os.listdir(config_path):
    ...
    additional_fields = {
        ...
        "adjacency_matrix_test_probabilities": [],
    ...
    adjacency_matrix_test_probabilities = np.zeros((num_steps, num_steps), dtype=float)
    ...
    for i in data[file_name]['steps'].keys():
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a helpful assistant specialized in partial ordering procedures. You need to choose possible next transition from given action, think where in the sequence this action may accure and what might come right after it. There might be a few possible answers. Choose from bag of actions, replay only the numbers of the actions . Take in to account the goal of the procedure. Give next possible action taking in to account chronological aspect. Return only the number of next possible action, no other words or signs"},
                {"role": "user", "content": "goal: "+ goal.format(str)+ " sequence:("+ value_step.format(str)+"), bag of actions: ("+steps_str+")"},
        ...
        for token_i in top_logprobs:
            ...
            probability_not_round = np.exp(token_i.logprob)
            probability_vec.append(probability_not_round)
        ...
        if len(indices)==1:
            ...
            probabilities = probability_vec[[indices[0]]]
        else:
            probability_vec_keep = [token_vec[k] for k in indices]
            probabilities = [probability_vec[k] for k in indices]
        ...
        for next_possible_actions,p_i in zip(converted_indices,probabilities):
            adjacency_matrix_test_probabilities[current_action[0],next_possible_actions] = p_i

        for response_str_i in response_str:
            adjacency_matrix_test_response[current_action[0],int(response_str_i)] = 1

    heat_maps_adjacency_matrix(adjacency_matrix_GT, adjacency_matrix_test, adjacency_matrix_test_probabilities, file_name)
    additional_fields['response'] = response_i
    additional_fields['adjacency_matrix_GT'] = adjacency_matrix_GT.tolist()
    additional_fields['adjacency_matrix_test_probabilities'] = adjacency_matrix_test_probabilities.tolist()
